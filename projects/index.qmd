...
The idea of `HealthDataScotland` arose when I found myself moving between
various addresses and trying to find GP practices in my local area. My first
thought was just to create an interactive dashboard that would allow users to
view GP practices in their local area and obtain their contact details. However,
this thought expanded to trying to make an application that was generic enough
that it was able present locations of any health care centre. I underwent a
process of trying to identify what public data sets were available and then
trying to use an object-oriented approach to design an R package that was able
to handle different health care centres and present their data in an appropriate
manner.
...
I tried to design the underlying R package to be generic enough to handle
different health care centres and present them appropriately in the app.
Currently `HealthDataScotland` only presents GP practices and hospitals. However
one could imagine future versions of the app presenting the location of other
health care settings such as dental practices or pharmacies.
